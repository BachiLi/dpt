...
void SetIsMoving(Path &path) {
    path.isMoving = false;
    for (const auto &vertex : path.camSurfaceVertex) {
    ...
    if (!path.isMoving) {
        for (const auto &vertex : path.lgtSurfaceVertex) {
            if (vertex.shapeInst.obj != nullptr && vertex.shapeInst.obj->IsMoving()) {
                path.isMoving = true;
                break;
            }
...
bool PerturbLensFirstVertex(const Scene *scene,
                            const Float roughnessThreshold,
                            const Vector3 &lensVertexPos,
                            const Float time,
                            UniPathState &pathState,
                            SurfaceVertex &surfVertex,
                            Vector3 &bsdfContrib) {
    ...
    bsdf->Evaluate(
        wi, isect.shadingNormal, ray.dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfPdfRev);
    if (bsdfContrib.isZero() || bsdfPdf <= Float(0.0) || fabs(cosWo) < Float(1e-2) ||
        fabs(Dot(ray.dir, isect.geomNormal)) < Float(1e-2)) {
        // Be very strict on grazing angle: lens perturbation sometimes produces
        // vertices on the same surfaces
        return false;
    }
    ...
    for (int camDepth = 0; camDepth < (int)path.camSurfaceVertex.size(); camDepth++) {
        ...
        if (camDepth == 0) {
            ...
            if (shapeInst.obj == nullptr) {
                ...
                Vector2 st;
                if (!shapeInst.obj->Intersect(
                        shapeInst.primID, path.time, pathState.raySeg, pathState.isect, st)) {
                    return;
                }
            }
        }
        ...
        Vector3 bsdfContrib;
        if (camDepth == 0) {
            if (!PerturbLensFirstVertex(scene,
                                        scene->options->roughnessThreshold,
                                        path.lensVertexPos,
                                        path.time,
                                        pathState,
...
bool PerturbLensLastVertex(const Scene *scene,
                           const Camera *camera,
                           const Float roughnessThreshold,
                           const Vector2 screenPos,
                           const Float time,
                           BidirPathState &pathState,
                           SurfaceVertex &curSurfVertex,
                           SurfaceVertex &nextSurfVertex,
                           Vector3 &dir,
                           Vector3 &bsdfContrib) {
    const BSDF *bsdf = curSurfVertex.shapeInst.obj->bsdf.get();
    curSurfVertex.useAbsoluteParam =
        BoolToFloat(bsdf->Roughness(curSurfVertex.shapeInst.st, curSurfVertex.bsdfDiscrete) >
                    roughnessThreshold);
...
bool PerturbLensFirstVertex(const Scene *scene,
    ...
    bsdf->Evaluate(
        wi, isect.shadingNormal, dir, shapeInst.st, bsdfContrib, cosWo, bsdfPdf, bsdfRevPdf);
    if (bsdfContrib.isZero() || bsdfPdf <= Float(0.0) || fabs(cosWo) < Float(1e-2) ||
        fabs(Dot(dir, isect.geomNormal)) < Float(1e-2)) {
        // Be very strict on grazing angle: lens perturbation sometimes produces
        // vertices on the same surfaces
        return false;
    }
...
void PerturbLensBidir(const Scene *scene,
    ...
    if (path.lgtDepth > 1) {
        ...
        for (int lgtDepth = 0; lgtDepth < (int)path.lgtSurfaceVertex.size(); lgtDepth++) {
            ...
            ShapeInst &shapeInst = surfVertex.shapeInst;
            if (!shapeInst.obj->Intersect(
                    shapeInst.primID, path.time, raySeg, lightPathState.isect, shapeInst.st)) {
                return;
            }
            ...
            if (lgtDepth == (int)path.lgtSurfaceVertex.size() - 2 && path.camDepth == 1) {
                if (!PerturbLensLastVertex(scene,
                                           camera,
                                           scene->options->roughnessThreshold,
                                           screenPos,
                                           path.time,
    ...
    for (int camDepth = 0; camDepth < (int)path.camSurfaceVertex.size(); camDepth++) {
        ...
        if (camDepth == 0) {
            ...
            if (shapeInst.obj == nullptr) {
                ...
                Vector2 st;
                if (!shapeInst.obj->Intersect(
                        shapeInst.primID, path.time, raySeg, camPathState.isect, st)) {
                    return;
                }
            }
        }
...
static const ADFloat *PerturbLensFirstVertex(const ADFloat *buffer,
                                             const ADVector3 &lensVertexPos,
                                             ADUniPathState &pathState) {
    const ADVector3 &wi = pathState.wi;
    const ADIntersection &isect = pathState.isect;
    ADRay &ray = pathState.ray;
...
static const ADFloat *PerturbLensLastVertex(const ADFloat *buffer,
                                            const ADMatrix4x4 &sampleToCam,
                                            const ADAnimatedTransform &camToWorld,
                                            const ADVector2 screenPos,
                                            const ADFloat time,
    ...
    if (maxLightDepth > 1) {
        ...
        for (int lgtDepth = 0; lgtDepth < maxLightDepth - 1; lgtDepth++) {
            ...
            if (mode == PathFuncMode::Lens && lgtDepth == maxLightDepth - 3 && maxCamDepth == 1) {
                buffer = PerturbLensLastVertex(buffer,
                                               sampleToCam,
                                               camToWorld,
                                               screenPos,
                                               time,
...
std::shared_ptr<const PathFuncLib> BuildPathFuncLibrary(const bool bidirectional,
    ...
    for (int maxCamDepth = 1; maxCamDepth <= maxDepth + 1; maxCamDepth++) {
        for (int maxLgtDepth = 0; maxLgtDepth <= (bidirectional ? maxDepth : 1); maxLgtDepth++) {
            ...
            std::string funcName =
                bidirectional ? GetFuncNameBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Full)
                              : GetFuncName(maxCamDepth, maxLgtDepth, PathFuncMode::Full);
            {
            ...
            }
            {
                ...
                if (f != nullptr) {
                    ...
                    std::cout << "[Warning] Can't find derivatives function:" << funcName
                              << std::endl;
                }
            }
            ...
            std::string lensFuncName =
                bidirectional ? GetFuncNameBidir(maxCamDepth, maxLgtDepth, PathFuncMode::Lens)
                              : GetFuncName(maxCamDepth, maxLgtDepth, PathFuncMode::Lens);
            if (maxCamDepth + maxLgtDepth > 3) {
                {
