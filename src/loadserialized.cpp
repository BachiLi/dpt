...
std::shared_ptr<TriMeshData> LoadSerialized(const std::string &filename,
    ...
    // bool faceNormals = flags & EFaceNormals;
    ...
    if (flags & EHasNormals) {
        ...
        if (fileDoublePrecision) {
            LoadNormal<double>(zs, data, invToWorld0, invToWorld1, isMoving);
        } else {
            LoadNormal<float>(zs, data, invToWorld0, invToWorld1, isMoving);
        }
    }
